...
| APS-400 | **DRAFT / INCOMPLETE** | CONF-001…015 documents exist; CONF-004/005/010 now have a repository-local draft execution path via `scripts/check-fix001-evidence.py` | Execute the matrix through a runner and gather objective PASS/FAIL evidence |
| APS-500 | **DRAFT / INCOMPLETE** | Fixture contract exists; `FIX-001` now supports repository-local draft evidence execution for CONF-004/005/010 | Publish canonical machine-readable fixture set and expected outputs |

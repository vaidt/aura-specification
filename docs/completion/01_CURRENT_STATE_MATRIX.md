...
| APS-400 | **DRAFT / INCOMPLETE** | CONF-001…015 documents exist; CONF-001/004/005/006/010 now have a repository-local draft execution path via `scripts/check-fix001-evidence.py` | Execute the matrix through a runner and gather objective PASS/FAIL evidence |
| APS-500 | **DRAFT / INCOMPLETE** | Fixture contract exists; `FIX-001` now supports a repository-local draft request/result/evidence/platform gate for CONF-001/004/005/006/010 | Publish canonical machine-readable fixture set and expected outputs |
